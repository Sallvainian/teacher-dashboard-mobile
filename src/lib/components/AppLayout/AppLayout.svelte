<script lang="ts">
	...
	import MobileAppLayout from './MobileAppLayout.svelte';
	...
	const debouncedDataLoad = debounce(() => {
		gradebookStore.ensureDataLoaded().catch(() => {});
	}, 100);

	// Debounced page status update to prevent rapid updates
	const debouncedPageStatusUpdate = debounce((...args: unknown[]) => {
		const pathname = args[0] as string;
		updatePageStatus(pathname).catch(() => {});
	}, 500);
...
{:else if $isAuthenticated}
	{#if $device.isMobile}
		<!-- Mobile Layout with Native Ionic Components -->
		<MobileAppLayout>
			{@render children?.()}
		</MobileAppLayout>
	{:else}
		<!-- Desktop Layout with Traditional Header/Sidebar -->
