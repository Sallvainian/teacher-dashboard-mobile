/**
 * Script to store Netlify documentation in Mem0 memory system
 * Run with: node scripts/store-netlify-docs.js
 */

import { memoryService } from '../src/lib/services/mem0Service.js';
  ...
  for (const doc of docs) {
    try {
      const memory = await memoryService.addMemory({
        userId: docUser.id,
        message: `Netlify Documentation: ${doc.title} - ${doc.description}`,
        metadata: {
...
async function storeAllDocumentation() {
  ...
  if (!memoryService.isInitialized()) {
    console.error('❌ Mem0 service is not initialized. Check API key configuration.');
    return;
  }
  ...
  await memoryService.addMemory({
    userId: docUser.id,
    message: 'Netlify is a web platform for building highly performant, secure, and reliable websites and web apps. This documentation covers all major areas: Development & Functions, Framework Integration, Build & Configuration, Data & Storage, Forms & User Input, Domain & HTTPS Management, Team & Account Management, and Billing & Usage.',
    metadata: {
      type: 'netlify_documentation',
      category: 'overview',
